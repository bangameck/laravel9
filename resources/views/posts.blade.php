...
@section('container')
        ...
        <div class="card mb-3">
            ...
            @else
                <img src="https://source.unsplash.com/1200x400?{{ $posts[0]->category->name }}"
                    alt="{{ $posts[0]->category->name }}" class="img-fluid mb-3">
            @endif
            <div class="card-body text-center">
                ...
                <p>
                    <small class="text-muted">
                        <b>By. <a href="/posts?author={{ $posts[0]->author->username }}"
                                class="text-decoration-none">{{ $posts[0]->author->name }}</a> in <a
                                href="/posts?category={{ $posts[0]->category->slug }}"
                                class="text-decoration-none">{{ $posts[0]->category->name }}</a></b>
                        ...
                        <div class="card">
                            ...
                            @if ($p->image)
                                <img src="{{ asset('storage/' . $p->image) }}" class="card-img-top"
                                    alt="{{ $p->category->name }}">
                            @else
                                <img src="https://source.unsplash.com/500x400?{{ $p->category->name }}"
                                    class="card-img-top" alt="{{ $p->category->name }}">
                            @endif

                            <div class="card-body">
                                <h5 class="card-title"><a href="/posts/{{ $p->slug }}"
                                        class="text-decoration-none">{{ $p->title }}</a></h5>
                                <p>
                                    <small class="text-muted">
                                        <b>By. <a href="/posts?author={{ $p->author->username }}"
                                                class="text-decoration-none">{{ $p->author->name }}</a> </b>
    ...
    @else
        <p class="text-center fs-4">Post <b>({{ $not_found }})</b> Not Found</p>
    @endif
